...
export default class SendAssetRow extends Component {
  ...
  clearUnsendableAssetError = () => {
    ...
    this.props.updateSendErrors({ unsendableAssetError: null });
  };
  ...
  selectToken = async (token) => {
    if (token && token.isERC721 === undefined) {
      ...
      if (updatedToken.isERC721) {
        const error = `Sending of collectible token ${token.symbol} is not currently supported`;
        this.props.setUnsendableAssetError(true);
        this.props.updateSendErrors({ unsendableAssetError: error });
      }
    }
